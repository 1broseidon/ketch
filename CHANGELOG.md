...
### Changed
- **Structural extraction.** Content selection now reads the page's own structure instead of scoring candidates: the landmark it declares (`main`, `article`), a document assembled from uniform sections, or the smallest element holding its prose. Site furniture is removed by what it is — navigation and other landmarks, hidden and collapsed controls, tables of contents, link rails, teaser grids, layout tables — and readability remains the fallback for a page that declares no structure. The rules are structural, not site-specific: there are no hostname or per-site selectors. On the 500-page, 70-site benchmark the default `clean` mode lifts passing checks from 3,176 to 3,468 of 3,573 (critical checks 1,692 → 1,822 of 1,831), macro token recall from 94.1% to 99.0% and precision from 98.1% to 99.4%. See [ADR-0005](design/adr/0005-structural-extraction.md).
