...
### Added
- **`extract_mode` config key.** `complete` (the default) keeps everything the page's structure does not condemn; `clean` also drops blocks by name and phrase — related-post rails, comment threads, share bars, cookie notices, "was this page helpful?" boxes — for the leanest markdown: 3,468 of 3,573 checks (critical 1,822 of 1,831), macro recall 99.0% and precision 99.4%. Cards and rails that together hold the page (a section front, a topic index) are kept in `clean` too. Set it with `ketch config set extract_mode clean` or `KETCH_EXTRACT_MODE`; `ketch config` reports the effective mode and an unknown value fails loud with the valid names. It applies everywhere the extractor runs — `scrape`, `search --scrape`, `crawl`, `extract`, and the MCP tools — and a non-default mode is folded into the page-cache key, so pages cached under one mode are never reused under the other.
- Developer extraction benchmark in `bench/`: 500 pinned pages from 70 websites across 28 content types, 3,573 source-backed checks, preserved historical cohorts and separate new-site reports, median/p95 timing, concurrent stress runs, live source-drift checks, per-worker fixture loading and explicit regression baselines for both extraction modes. It is its own Go module (`go -C bench run . check`), and the corpus itself is a 22 MB tarball that `setup` downloads and verifies (`bench/archive.json`) rather than a thousand files in the repository. See the [benchmark guide](bench/README.md).
- **Tags: bookmarks for agent workflows.** Save sources from search, code, docs, scrape and crawl under shared project/topic labels, then revisit their URLs, titles and descriptions in later sessions. `tag add/show/list/remove` also ships as the sixth MCP tool; all five research tools accept `tag`. Bookmarks outlive page-cache expiry and clearing, and full bodies remain separate.
- **Independent portable storage.** `tags.db` uses the native configuration directory on Linux, macOS and Windows, with `KETCH_TAGS_PATH` as a filename override. Short-lived index handles keep bookmarks usable while crawls or MCP processes hold `cache.db`. Unknown warmth is reported as `cache_status: unavailable`. The unreleased in-cache tag layout is not automatically imported.
- **Bounded tag views.** CLI `tag show --limit` and MCP `tag.limit` default to 50 newest entries; 0 selects all. Text reports truncation; JSON/MCP add `shown` and `cache_status`, retaining whole-tag totals.
- **Tag index visibility.** `ketch cache` reports the tag index alongside the page cache (path, tag and entry counts, whether another process holds its lock), and `ketch doctor` gains an informational `tags` row — openable, how much it holds — that never gates the exit code, since bookmarks are optional.

### Changed
- **Structural extraction.** Content selection now reads the page's own structure instead of scoring candidates: the landmark it declares (`main`, `article`), a document assembled from uniform sections, or the smallest element holding its prose. Site furniture is removed by what it is — navigation and other landmarks, hidden and collapsed controls, tables of contents, link rails, teaser grids, layout tables — and readability remains the fallback for a page that declares no structure. The rules are structural, not site-specific: there are no hostname or per-site selectors. On the 500-page, 70-site benchmark this lifts passing checks from 3,176 to 3,475 of 3,573 (critical checks 1,692 → 1,825 of 1,831) and macro token recall from 94.1% to 99.4% at unchanged precision (98.1%). See [ADR-0005](design/adr/0005-structural-extraction.md).

  *Upgrading:* the markdown for most pages changes — usually more of the page, in the page's own order. Pages already in the page cache are served as cached until they expire; run `ketch cache clear` to re-extract them on the next fetch. Anything that keyed on the old readability output (snapshot tests, stored digests) should be regenerated.

### Fixed
- **A plain scrape no longer writes to `tags.db`.** Storing a page checked for bookmarks to refresh through a read-write open of the tag index on every cache miss — two extra opens and four extra `fdatasync` calls per page, and a one-second stall plus a stray tag warning whenever another process held the index. The refresh now looks the URL up read-only with a 100 ms lock budget and writes only when the URL is actually bookmarked; a locked index is skipped silently. Explicit `ketch tag` operations keep their one-second wait and their warnings.
- **`tag` output parity.** The MCP `tag` tool's `entries`, `shown` and `cached` appear only for `show` (no more `"entries":0` on add/list/remove). `tag remove <tag> <url…>` now reports the URLs that were not under the tag as `missing` — in the CLI summary, `--json` and MCP — instead of claiming success for them; removing a whole tag says `dropped tag <name> (N entries)`.
- **`tag add` validates its URLs.** Bookmarks must be absolute `http://` or `https://` URLs with a host, the rule crawl and search already apply; `javascript:`, `ftp:`, bare words and the empty string are rejected as exit 2 / `[validation]` before anything is written, so a batch with one bad URL tags nothing.
- **MathML formulas come out as TeX.** A `<math>` element with an `application/x-tex` annotation (MathJax, KaTeX, MediaWiki) becomes `$…$`, or `$$…$$` for display math, with the TeX untouched — previously the annotation was escaped as prose and MediaWiki's wrapper stripped a brace.
- **Fenced code inside a blockquote keeps its quote prefix on every line.** The converter left all but the first line unquoted, which ended the quote mid-listing and left the fence unclosed.
- **Highlighter line numbers stay out of code blocks.** Pygments `linenos` spans, Sphinx and Rouge gutter tables and highlight.js number cells no longer leak digits into the code.
- **A collapsed panel a control names is kept.** An `aria-hidden="true"` element referenced by `aria-controls` (a disclosure, a tab panel) is content someone folded away, not decoration, and survives extraction.
- **Legacy charsets are decoded before extraction.** A page served as windows-1252, Shift_JIS or any other non-UTF-8 encoding — declared in `Content-Type` or `<meta charset>`, or undeclared and sniffed the way browsers do — no longer reaches the extractor as raw bytes that come out as replacement characters; em dashes and curly quotes on an undeclared windows-1252 essay survive.
- Preserve highlighted code comments, line boundaries and language labels during HTML extraction. Go API declarations and npm-style YAML blocks no longer lose syntax-token text or concatenate rendered lines. Selector scrapes resolve links against the full fetch URL; raw fallback links retain the page directory. See the [extraction investigation](design/extraction-accuracy.md) for measured coverage and remaining content-selection limits.
- Bookmark identity uses the source URL, so changing cookies, User-Agent or rewrite settings does not duplicate a source or prevent removal. Cold re-adds preserve metadata; fetch-time backfill survives cache clearing and cannot resurrect deleted memberships.
- Selector scrapes, direct-library docs and search-with-scrape consistently record tags across CLI and MCP. Failed optional bookmark writes retain research output and emit CLI stderr diagnostics or MCP `warnings`; invalid tag names and missing removals have consistent errors.
- Correct the cache-clear documentation: clearing frees storage for reuse without shrinking the bbolt file. It leaves the separate bookmark index intact. See [ADR-0004](design/adr/0004-tagged-cache-corpus.md).
