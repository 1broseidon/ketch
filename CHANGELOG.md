...
- Developer extraction benchmark in `bench/`: 500 pinned pages from 70 websites across 28 content types, 3,573 source-backed checks, preserved historical cohorts and separate new-site reports, median/p95 timing, concurrent stress runs, live source-drift checks, per-worker fixture loading and explicit regression baselines for both extraction modes. It is its own Go module (`go -C bench run . check`), so the 54 MB corpus never ships inside `go install github.com/1broseidon/ketch@latest`. See the [benchmark guide](bench/README.md).
