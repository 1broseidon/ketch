...
### Added
- **Local docs backend** (`ketch docs -b local`), replacing the long-standing unimplemented FTS5 stub. `ketch docs add <name> <url>` pulls a documentation site onto the machine and `ketch docs -b local "query"` searches it offline with no API key. `add` discovers the cheapest reliable source itself — `llms-full.txt` at the site root, then `llms.txt` as a link list, then sitemaps from `robots.txt` or `/sitemap.xml`, then a bounded same-host BFS crawl — scopes list sources and crawls to the URL's path prefix (`--prefix`, `/` for the whole host), and fetches through the same scrape pipeline and page cache as `ketch scrape`, so JS-shell pages fall back to the browser and PDFs are extracted. Each page is split at its headings into sections carrying a breadcrumb and anchor, sections whose text recurs verbatim on three or more pages (sidebars, footers) are dropped as boilerplate, and the rest are indexed in SQLite FTS5 via the pure-Go `modernc.org/sqlite` (CGO stays off). Search is BM25-ranked with heading and breadcrumb weighted above body text; free text is reduced to plain terms (FTS5 syntax never reaches the engine), matched as all-terms first and any-term when that finds nothing, so question-shaped queries still land on the closest sections. Results carry `source: local`, the library's `version` label, and a URL with the heading anchor. `--dry-run` reports the discovered source and page count without fetching or writing; the summary counts `unrendered` pages (JS shells no browser could render, so their content may be partial) alongside fetched/skipped/failed; `--max-pages` (500), `--depth` (5), `--concurrency` (8), `--sitemap`, `--version`, `--no-cache`, `--cookie-file`, and `--user-agent` bound and tune the pull. Re-adding a name replaces the library atomically.

  Libraries are stored per user in `$XDG_DATA_HOME/ketch/docs` (`~/Library/Application Support/ketch/docs` on macOS, `%LOCALAPPDATA%\ketch\docs` on Windows; `docs_dir` / `KETCH_DOCS_DIR` overrides). Run inside a project (a tree with `.ketch/docs.json` or a git repository) `add` records the library in `.ketch/docs.json` so bare `-b local` searches default to the project's libraries and `ketch docs sync` rebuilds them on another machine; `--global` skips the attachment. `ketch docs list` shows every library with page/section counts and its attachment, `ketch docs remove <name>` deletes one. `--library <name>` targets one local library, `--resolve <text>` lists stored libraries by name substring, and `--tokens` bounds `--library` output at about four characters per token. The backend is advertised everywhere `context7` is — `ketch config`'s `available_doc_backends`, the MCP `docs` tool's description and `backend` parameter, `ketch doctor` (`skipped: no libraries yet` until the first `add`) — and selecting it before any library exists is a precondition error (exit 5) naming the `add` command. Exit codes from `add`: `2` for a bad name or seed URL, `3` when the site fetched but produced nothing indexable, `4` when discovery or every fetch failed.

  Design record: [ADR-0004](design/adr/0004-local-docs-corpus.md). The `crawl` package is now also consumed as a library by the ingest pipeline (`crawl.FetchSitemap` is exported, and `crawl.Result` gains an additive `FetchSource` field — `http`, `http_shell`, or `browser` — for library callers; CLI and MCP crawl output is unchanged), `scrape.Scraper.CachedScrapeSource` is the source-reporting form of `CachedScrape`, and `extract/sections` is a new leaf package holding the heading-based markdown chunker.
