...
Every keyed backend also accepts a pool of keys (`brave_api_keys`, `exa_api_keys`, `firecrawl_api_keys`, `keenable_api_keys`, `tavily_api_keys`, `serpbase_api_keys`); ketch picks one at random per request and retries once with a different key on `401`/`429` (`402` for Firecrawl; SerpBase rotates on its HTTP-200 business codes `1001`/`1029`). See [multiple API keys](/guide/configuration#multiple-api-keys-per-provider).
