...
| --- | --- | --- | --- |
| search | ddg, searxng (self-hosted), exa, firecrawl, keenable, parallel | brave, tavily, serpbase, serply | `ketch config set brave_api_key <key>` / `tavily_api_key` / `serpbase_api_key` / `serply_api_key` (optional `firecrawl_api_key` lifts Firecrawl's hosted cap) |
