...
- A repository the backend lacks: sourcegraph → `[not_found]` / exit 3 naming the other backends. grepapp indexes a subset of public repos, so an empty `repo` search there comes back with a `[not_found]` warning (`repo_may_be_unindexed`), not an error. Try another backend before concluding the code does not exist.
